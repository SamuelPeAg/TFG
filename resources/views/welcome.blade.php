...
@section('content')
    ...
    <div class="excel-grid min-h-screen bg-white text-gray-900 overflow-hidden">
        ...
        <section id="calendario-clases" class="py-32 bg-white reveal">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                eventClick: function (info) {
                    ...
                    if (confirm('¿Deseas reservar la clase "' + props.clase + '" con ' + props.entrenador + '?')) {
                        // Redirigir al dashboard para procesar la reserva con seguridad (CSRF, etc.)
                        window.location.href = "{{ route('cliente.dashboard') }}";
                    }
                },
                eventContent: function (arg) {
                    let italicEl = document.createElement('div');
                    italicEl.innerHTML = `<div class="p-1 overflow-hidden">
                            <div class="font-bold text-[10px] uppercase truncate">${arg.event.title}</div>
                            <div class="text-[9px] opacity-80">${arg.event.extendedProps.entrenador}</div>
                        </div>`;
