...
<body class="bg-slate-50 text-slate-800">
    ...
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 p-4 md:p-8 lg:ml-[260px] ml-0 fade-in w-full transition-all duration-300">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div>
                    <h1
                        class="text-2xl md:text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-gray-700 to-gray-900">
                        Gestión de Nóminas
                    </h1>
                    <p class="text-slate-400 mt-1 font-medium text-sm md:text-base">Supervisión y control de pagos a
                        entrenadores</p>
                </div>
            ...
            @if(session('success'))
                <div
                    class="bg-green-100 border border-green-200 text-green-800 px-4 py-3 rounded-xl mb-6 flex items-center gap-3 shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-10">
                <!-- Card 1: Borradores -->
                <div
                    class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 relative overflow-hidden group hover:shadow-md transition-all">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <div
                            class="w-12 h-12 bg-orange-50 rounded-2xl flex items-center justify-center text-orange-500 text-xl group-hover:scale-110 transition-transform">
                ...
                <div
                    ...
                    <div class="flex justify-between items-center">
                        <div>
                            <h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Pendiente Pago
                            </h4>
                        ...
                        <div
                            class="w-12 h-12 bg-teal-50 rounded-2xl flex items-center justify-center text-teal-600 text-xl group-hover:scale-110 transition-transform">
                ...
                <div
                    ...
                    <div class="flex justify-between items-center">
                        <div>
                            <h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Pagado
                                (Histórico)</h4>
                        ...
                        <div
                            class="w-12 h-12 bg-green-50 rounded-2xl flex items-center justify-center text-green-600 text-xl group-hover:scale-110 transition-transform">
                        ...
                        <table class="w-full text-left min-w-[800px]">
                            <thead class="bg-slate-50 border-b border-slate-200">
                                <tr>
                                    ...
                                    <th class="py-4 px-6 text-xs font-bold text-slate-500 uppercase text-right">Acciones
                                    </th>
                                </tr>
                            </thead>
                                    ...
                                    <tr class="hover:bg-slate-50/80 transition-colors search-item">
                                        <td class="py-4 px-6">
                                            <div class="flex items-center gap-3">
                                                <div
                                                    class="w-9 h-9 bg-indigo-50 text-indigo-600 rounded-full flex items-center justify-center font-bold text-sm">
                                                    {{ substr($nomina->user->name, 0, 1) }}
                                                </div>
                                                <span
                                                    class="font-semibold text-slate-700 name-cell">{{ $nomina->user->name }}</span>
                                            </div>
                                        </td>
                                        <td class="py-4 px-6 text-slate-600 font-medium">{{ $nomina->mes }}/{{ $nomina->anio }}
                                        </td>
                                        <td class="py-4 px-6 text-slate-800 font-bold text-lg">
                                            {{ number_format($nomina->importe, 2) }} €</td>
                                        <td class="py-4 px-6">
                                            <span
                                                class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-orange-50 text-orange-600 border border-orange-100">
                                                <i class="fas fa-pencil-alt text-[10px]"></i> Borrador
                                            </span>
                                        </td>
                                        <td class="py-4 px-6 text-right">
                                            <div class="flex items-center justify-end gap-2">
                                                <button onclick="abrirModalRevision(this)" data-id="{{ $nomina->id }}"
                                                    data-userid="{{ $nomina->user_id }}" data-name="{{ $nomina->user->name }}"
                                                    data-importe="{{ $nomina->importe }}" data-mes="{{ $nomina->mes }}"
                                                ...
                                                <form action="{{ route('admin.nominas.destroy', $nomina->id) }}" method="POST"
                                                    onsubmit="return confirm('¿Estás seguro de que deseas eliminar este borrador?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                                                        class="w-10 h-10 flex items-center justify-center bg-red-50 text-red-500 rounded-xl hover:bg-red-100 transition-colors shadow-sm"
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse($historial as $nomina)
                                                        <tr class="hover:bg-slate-50/50 transition-colors search-item">
                                                            ...
                                                            <td class="py-4 px-6 font-bold text-slate-700">{{ number_format($nomina->importe, 2) }}
                                                                €</td>
                                                            <td class="py-4 px-6">
                                                                ...
                                                                @else
                                                                    <span
                                                                        class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-sky-50 text-sky-700 border border-sky-100">
                                    ...
                                    'detalles' => $nomina->detalles
                                                                        ...
                                                                        class="w-9 h-9 rounded-lg flex items-center justify-center bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors"
                                                                        title="Ver Detalles">
                                                                    ...
                                                                    @if($nomina->estado_nomina == 'pendiente_pago')
                                                                        <form action="{{ route('admin.nominas.pagar', $nomina->id) }}" method="POST">
                                                                            @csrf
                                                                            <button type="submit"
                                                                                class="w-9 h-9 rounded-lg flex items-center justify-center bg-green-50 text-green-600 hover:bg-green-100 transition-colors"
                                                                    ...
                                                                    <form action="{{ route('admin.nominas.destroy', $nomina->id) }}" method="POST"
                                                                        onsubmit="return confirm('¿Seguro que deseas eliminar esta nómina?');">
                                                                        @csrf @method('DELETE')
                                                                        <button
                                                                            class="w-9 h-9 rounded-lg flex items-center justify-center bg-red-50 text-red-500 hover:bg-red-100 transition-colors"
                                                                            title="Eliminar">
                                                                            <i class="fas fa-trash-alt"></i>
                                                                        </button>
                                                                    </form>
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="py-8 text-center text-slate-400 italic">No hay historial
                                        disponible.</td>
                                </tr>
                            @endforelse
                        </tbody>
    ...
    <div id="modalRevision"
        class="fixed inset-0 bg-black/40 backdrop-blur-sm z-[2000] hidden items-center justify-center fade-in overflow-y-auto">
        <div class="bg-white w-full max-w-2xl rounded-2xl p-8 shadow-2xl relative my-8">
            ...
            <div class="text-center mb-6">
                <div
                    class="w-16 h-16 bg-orange-50 text-orange-500 rounded-full flex items-center justify-center mx-auto mb-4 text-2xl">
            ...
            <form id="formRevision" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    {{-- Bloque 1: Datos Base --}}
                    <div class="space-y-4">
                        <h4 class="text-sm font-bold text-slate-400 uppercase tracking-widest border-b pb-2">Datos Base
                        </h4>

                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-1">Entrenador</label>
                            <div id="modalEntrenadorNombre"
                                class="w-full p-3 rounded-xl border-2 border-slate-100 font-bold text-slate-700 bg-slate-50">
                                --
                            </div>
                    ...
                    <div class="space-y-4">
                        <h4 class="text-sm font-bold text-slate-400 uppercase tracking-widest border-b pb-2">Desglose
                            Económico</h4>

                        <div class="flex items-center justify-between gap-4">
                            <label class="text-xs font-bold text-slate-500">Salario Bruto (€)</label>
                            <input type="number" step="0.01" id="editBruto"
                                class="w-32 p-2 rounded-lg border border-slate-200 text-right font-bold text-slate-700 focus:outline-none focus:border-brand-teal">
                        </div>
                        ...
                        <div class="flex items-center justify-between gap-4 text-red-500">
                            ...
                            <input type="number" step="0.1" id="editIRPF" value="0"
                                class="w-16 p-2 rounded-lg border border-slate-200 text-right font-bold focus:outline-none">
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-8 p-6 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
                    <div class="flex justify-between items-center mb-4">
                        <h4 class="text-xs font-bold text-slate-500 uppercase tracking-widest">Conceptos Adicionales
                            (Extras/Bonos)</h4>
                        <button type="button" onclick="agregarFilaExtra()"
                            class="text-xs font-bold bg-slate-800 text-white px-3 py-1.5 rounded-lg hover:bg-slate-700 transition-all">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                    <div class="bg-teal-50 p-6 rounded-2xl border border-teal-100">
                        <label class="block text-xs font-bold text-teal-600 uppercase mb-1">SALARIO NETO (A
                            Pagar)</label>
                        ...
                        <p class="text-[10px] text-teal-500 font-bold mt-2">Este importe es el que verá el entrenador
                        </p>
                    </div>
                ...
                <div class="mb-8">
                    ...
                    <div id="linkArchivoActual" class="mt-2 text-sm text-center hidden">
                        <a href="#" target="_blank" class="text-brand-teal font-bold hover:underline"><i
                                class="fas fa-file-pdf mr-1"></i> Ver PDF actual</a>
                    </div>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-8">
                    <button type="submit" name="accion" value="guardar" 
                            class="flex items-center justify-center gap-2 py-4 px-6 rounded-xl font-bold bg-slate-100 text-slate-600 hover:bg-slate-200 transition-all border border-slate-200 shadow-sm">
                        <i class="fas fa-save"></i>
                        <span>Guardar Borrador</span>
                    </button>
                    <button type="submit" name="accion" value="confirmar" 
                            class="flex items-center justify-center gap-2 py-4 px-6 rounded-xl font-bold bg-slate-900 text-white shadow-xl hover:bg-slate-800 transition-all transform hover:-translate-y-0.5 active:scale-95">
    ...
    <div id="modalDetalleAdmin"
        class="fixed inset-0 bg-black/40 backdrop-blur-sm z-[1000] hidden items-center justify-center fade-in">
        <div class="bg-white w-full max-w-lg rounded-2xl p-8 shadow-2xl relative">
            ...
            <div class="space-y-4 mb-8">
                ...
                <div class="flex justify-between items-center bg-slate-50 p-4 rounded-xl">
                    ...
                    <span id="modalDetalleEstadoBadge"
            ...
            <a id="btnDescargarPDFAdmin" href="#" target="_blank"
                class="flex items-center justify-center gap-3 w-full py-4 bg-slate-800 text-white rounded-xl font-bold shadow-lg hover:shadow-xl transition-all hover:-translate-y-1">
