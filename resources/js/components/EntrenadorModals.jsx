...
export default function EntrenadorModals({ 
  isOpen, 
  onClose, 
  onSave, 
  entrenador = null,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await onSave(formData, mode, entrenador?.id);
      onClose();
    } catch (err) {
        ...
        <div className="p-8">
          <div className="text-center mb-8">
            ...
            <h2 className="text-2xl font-black text-slate-800 tracking-tight">
              {mode === 'edit' ? 'Editar Entrenador' : 'Añadir Entrenador'}
            </h2>
            <p className="text-slate-500 text-sm mt-1 font-medium">
              {mode === 'edit' ? 'Modifica los datos del entrenador seleccionado' : 'Introduce la información para registrar al nuevo entrenador. Se le enviará un correo electrónico.'}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {mode === 'create' && (
                ...
                <div className="space-y-1.5">
                    ...
                    {errors.nombre && <p className="text-rose-500 text-[10px] font-bold mt-1 ml-1">{errors.nombre[0]}</p>}
                </div>
            ...
            {mode === 'edit' && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="relative group">
                        <i className="fas fa-check-double absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-[#E65C9C] transition-colors"></i>
                        <input
                        type="password"
                    ...
                    {errors.password_confirmation && <p className="text-rose-500 text-[10px] font-bold mt-1 ml-1">{errors.password_confirmation[0]}</p>}
                    </div>
                </div>
                </>
            )}
            ...
            <div className="pt-6">
              ...
              >
                {loading ? (
                  ...
                  <span>{mode === 'edit' ? 'ACTUALIZAR DATOS' : 'REGISTRAR ENTRENADOR'}</span>
                )}
              </Button>
            </div>
          </form>
