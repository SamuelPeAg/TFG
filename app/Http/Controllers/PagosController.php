        ...
        try {
            ...
            $query = Pago::select('id', 'user_id', 'fecha_registro', 'nombre_clase', 'tipo_clase', 'centro', 'capacidad_maxima', 'importe', 'metodo_pago')
                ->with([
                    ...
                    'entrenadores:id,name,foto_de_perfil', 
            ...
            foreach ($grouped as $key => $grupo) {
                ...
                $events[] = [
                    ...
                    'extendedProps' => [
                        ...
                        'entrenadores' => $grupo->flatMap->entrenadores->unique('id')->map(fn($t) => ['id' => $t->id, 'name' => $t->name, 'initial' => strtoupper(substr($t->name, 0, 1)), 'photo' => $t->photo ?? null])->values(),
